...
class CommentForm {
    ...
    init() {
        ...
        var post_btn = this.formEl.elements.post;
        var preview_btn = this.formEl.elements.preview;
        ...
        // bootstrap5 theme
        var reply_buttons = document.getElementsByClassName("reply-to-comment");
        for (var i = 0; i < reply_buttons.length; i++) {
            reply_buttons[i].addEventListener('click', ev => this.show_reply_form(ev));
        }
        this.expand_comment_field_on_focus();
    }
    ...
    show_reply_form(ev) {
        ev.preventDefault();
        document.querySelectorAll('.reply-to-comment-form').forEach(e => e.remove());
        var comment_id = ev.currentTarget.parentElement.parentElement.id.replace('cm-feedback-', '');
        var reply_form = document.getElementById('reply_form');
        reply_form.elements.id_comment.style.minHeight = null;
        var new_reply_form = reply_form.cloneNode(true);
        new_reply_form.id_reply_to.value = comment_id;
        new_reply_form.classList.add("reply-to-comment-form");
        ev.currentTarget.parentElement.parentElement.parentElement.parentElement.after(new_reply_form);
        // init
        this.formEl = document.body.querySelector(".reply-to-comment-form");
        var post_btn = this.formEl.elements.post;
        var preview_btn = this.formEl.elements.preview;
        post_btn.addEventListener("click", (_) => this.post("post"));
        preview_btn.addEventListener("click", (_) => this.post("preview"));
        ...
        preview_btn.type = "button";
        this.expand_comment_field_on_focus();
        new_reply_form.elements.id_comment.focus();
    }

    expand_comment_field_on_focus() {
        const textareas = document.getElementsByName("comment");
        textareas.forEach((textarea) => {
            textarea.addEventListener("focus", (ev) => {
                ev.currentTarget.style.minHeight = "200px";
                ev.currentTarget.form.querySelector(".comment-user-info").classList.remove("d-none");
            });
        });
    }
    ...
    async handle_preview_comment_response(response) {
        ...
        if (response.status === 200) {
            ...
            htmlObject.innerHTML = data.html;
            this.formEl.after(htmlObject);
            this.init();
            if (data.field_focus) {
